...
class K8sCronJob extends Command {
  ...
  async execute() {
    try {
      ...
      if (!alreadyExists) {
        ...
        this._obj = await this._k8sClient.get(
          'cronjob',
          this._options.name,
          this._options.namespace
        );
      ...
      this._logger.error(
        `An error occurred while creating cron job: ${e.message.toString()}\n\n${JSON.stringify(
          e
        )}\n\n${e.stack}`
      );
    }
  ...
  async stop() {
    ...
    try {
      ...
      this._logger.error(
        `An error occurred while deleting cron job: ${e.message.toString()}\n\n${JSON.stringify(
          e
        )}\n\n${e.stack}`
      );
    }
