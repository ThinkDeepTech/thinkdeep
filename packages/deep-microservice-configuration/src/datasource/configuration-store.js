...
class ConfigurationStore extends MongoDataSource {
    ...
    async readConfigurationForUser(userEmail) {
        ...
        try {
            const configurations = await this.collection.find({userEmail}).limit(1).toArray();
    ...
    async updateConfigurationForUser(userEmail, configuration) {
        ...
        try {
            await this.collection.updateOne({userEmail}, { $set: { observedEconomicEntities: configuration.observedEconomicEntities }});
        } catch (e) {
